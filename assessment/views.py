...
def createassessment(request):
    if request.method == 'POST':
        ...
        faculty=Faculty.objects.get(user=id)
        print(faculty)
        if form.is_valid():
...
class AssessmentUpdate(PermissionRequiredMixin, UpdateView):
    ...
    login_url = 'denied'
    model = Assessment
    fields = ['course', 'assessment_type', 'start_date', 'duration', 'year']

@permission_required('assessment.change_assessment', login_url='denied')
def questionupdate(request,pk):
    if request.method=="POST":
        p = AssessmentQuestion.objects.get(pk=pk)
        z = Assessment.objects.get(id=p.assessment_id)
        list = z.course
        form = QuestionForm(list, request.POST,instance=p)
        if form.is_valid():
            post = form.save(commit=False)
            post.assessment = Assessment.objects.get(pk=p.assessment_id)
            post.save()
            form.save_m2m()
        ...
        z = Assessment.objects.get(id=p.assessment_id)
        list = z.course
        question_form = QuestionForm(list, instance=p)
        return render(request, 'assessment/assessmentquestion_form.html', {'form': question_form})
...
def createquestion(request, assessment_id):
    if request.method == "POST":
        p = Assessment.objects.get(id=assessment_id)
        list = p.course
        form = QuestionForm(list,request.POST)
        if form.is_valid():
            post = form.save(commit=False)
...
def result_faculty(request):
    ...
    Students = Student.objects.raw("select distinct student_student.id from student_student, course_course, "
                                   "course_courseenrollment where "
                                   "student_student.user_id=course_courseenrollment.student_id "
                                   "and student_student.program_id =%s and course_course.semester =%s "
                                   "and course_courseenrollment.year=%s and "
                                   "course_course.id=course_courseenrollment.course_id    ", [program_obj, semester_obj, year_obj])
    ...
    for Student_obj in Students:
        for Course_obj in Course.objects.raw("select course_course.id from course_course, course_courseprogram, course_courseenrollment "
            ...
            for Assessment_obj in Assessment.objects.raw("select assessment_assessment.id from assessment_assessment,course_courseenrollment "
                                                         ...
                                                         "and assessment_assessment.course_id=course_courseenrollment.course_id and assessment_assessment.result_completed=1", [Student_obj.user_id,Course_obj.id,year_obj]):
...
def result_course(request, c_id):
    ...
    Students = Student.objects.raw("select distinct student_student.id from student_student, course_course, course_courseenrollment where student_student.program_id =%s and course_course.semester =%s "
                                   "and course_course.id=course_courseenrollment.course_id and course_courseenrollment.year=%s and student_student.user_id=course_courseenrollment.student_id",[program_obj, semester_obj, year_obj])
    p = []
    l = []
    max_marks = []
    for student_obj in Students:
        for assessment_obj in Assessment.objects.raw("select assessment_assessment.id from assessment_assessment,course_courseenrollment "
                                                     ...
                                                     "assessment_assessment.course_id=%s and assessment_assessment.year=%s and "
                                                     "assessment_assessment.year=course_courseenrollment.year "
                                                     "and assessment_assessment.course_id=course_courseenrollment.course_id and assessment_assessment.result_completed=1 "
...
def result_student(request, stu_id):
    ...
    for course_obj in Courses:
        ...
        for assessment_obj in Assessment.objects.raw("select * from assessment_assessment,course_courseenrollment "
                                                     ...
                                                     "assessment_assessment.course_id=%s and assessment_assessment.year=%s and "
                                                     "assessment_assessment.year=course_courseenrollment.year "
                                                     "and assessment_assessment.course_id=course_courseenrollment.course_id and "
                                                     "assessment_assessment.result_completed=1", [stu_id,course_obj.id,year_obj]):
            count = count+1
            if assessment_obj not in l:
...
def attainment_calculate(request):
    ...
    for i in related_assignment:
        ...
        for q in questions_in_i_th_assessment:
            ...
            print('total gain marks', total_gain_marks)
            print('hi',q.max_marks)
