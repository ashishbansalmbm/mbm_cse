...
def Enroll_Student(request,ca_id):
    ...
    sem = crse.semester
    pro = CourseAvailable.objects.raw("select * from course_courseavailable, course_courseprogram where course_courseavailable.id=%s and "
    ...
    if request.method == "POST":
        ...
        for i in m:
            ...
            print("ram",obj.student)
            obj.year = year
            obj.course = Course.objects.get(id =courses.course.id)
            obj.save()
            display = 1
            title = 'Done!!'
            message = 'Your Enrollment Was Successful!!'
            context = {'display': display, 'message': message, 'title': title, "students": student_list,}
