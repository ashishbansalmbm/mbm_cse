...
def AcademicProject(request):
    ...
    tid = Team.objects.filter(project_id__isnull=True).values('project_id')
    a_pro_list = Project.objects.filter(type='IDEA', report=0, added_by=uid).exclude(status='Completed', id__in=tid)
...
def ShowTeams(request, pro):
    teams = Team.objects.filter(project_id__isnull=True).order_by('id')
...
def VerifyProject(request):
    ...
    team_list = User.objects.raw("select project_project.title, project_team.team_name,project_team.id, "
                                 "first_name,last_name,project_project.id as pid"
                                 ",project_project.resources from project_project ,project_team,auth_user "
                                 "where project_project.id=project_team.project_id and project_project.type='ACA_V' and "
                                 "project_project.report=0 and project_project.supervisor_id is NULL and team_leader_id=auth_user.id")
...
def ProjectVerified(request, id):
    ...
    obj.supervisor = User.objects.get(id=request.user.id)
    obj.type = "ACA_A"
    obj.save()
...
def MyProject(request):
    my_pro_added = Project.objects.filter(added_by=request.user.username).exclude(type='MNR')
    my_pro_supervisor = Project.objects.filter(supervisor_id=request.user.id).order_by('title')
    ...
    if request.method == 'POST':
        ...
        if obj2[i]:
            ...
            for mem in obj2:
                ...
                x.team_id = Team.objects.latest('id')
                print((x.team_id))
                x.save()
                i = i+1
            return redirect(reverse('project:index'))
        else:
            messages.error(request,"OOPS! Seems like some field is left blank.")
    else:
...
def ApplyTeamIndex(request, pro):
    if request.method == 'POST':
        ...
        if obj2[i]:
            ...
            messages.error(request,"OOPS! Looks like some field is left blank.")
    else:
...
def featured_projects(request):
    obj = Project.objects.filter(featured=1).order_by('title')
...
def ApplyForAcademicProject(request):
    obj = User.objects.get(id=request.user.id)
    # print(obj.username)
    pro = Project.objects.filter(type='ACA_V', added_by=obj.username).order_by('title')
    teams = Team.objects.filter(team_leader_id=request.user.id,project_id__isnull=True).order_by('team_name')
...
def AllotApplied(request):
    ...
    obj.project = Project.objects.get(id=pro)
    obj.save()
    messages.success(request,'Alloted the selected project to the team!')
    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))